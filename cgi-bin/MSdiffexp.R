...
generate_Venn_diagrams <- function(results_intensities, replicate_descs) {
  ...
  if (n_bioreps>5)
  {
    ...
    result <- tryCatch({
      ...
      VennPalette <- c("#00a8ff", "#9c88ff", "#fbc531", "#4cd137", "#487eb0")
      g <- grid.newpage()
      venn.plot <- venn.diagram(lst_prots_per_brep_quantified, NULL , fill=VennPalette[1:n_bioreps], lwd=1, col=VennPalette[1:n_bioreps], margin = 0.13, cex=2.5, cat.cex=2.5)
      g <- grid.draw(venn.plot)
